...
class HouseVoiceExecutionHistory:
    ...
    def get_version(self, chain_id: str, version_num: int) -> dict[str, Any] | None:
        ...
        versions = self.store.data.get(f"versions_{chain_id}", {})
    ...
    async def async_save_version(
        ...
        if versions_key not in self.store.data:
            self.store.data[versions_key] = {}
        
        self.store.data[versions_key][str(version_num)] = {
            "timestamp": time.time(),
