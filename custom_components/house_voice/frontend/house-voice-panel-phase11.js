...
/* ── Responsive ── */
    ...
    if (this._loading) {
      return `
        <div class="loading-container">
          ...
          <div class="loading-text">📊 Indlæser analytik...</div>
        </div>
      `;
    }
      ...
      <div class="analytics-wrapper">
        <div class="analytics-header">
          <div class="analytics-title">
            ...
            <p class="subtitle">Kæde-eksekveringer, performance-metriker og trends</p>
          </div>
          <div class="analytics-actions">
            <button class="btn btn-icon" id="analytics-refresh" title="Opdater analytik-data">
              🔄 Opdater
            </button>
          </div>
        </div>
        ...
        <div class="analytics-filters">
          <div class="filter-group">
            <label for="analytics-chain" class="filter-label">
              <span class="label-text">Kæde</span>
              <span class="tooltip-icon" title="Filtrer efter specifikk kæde">?</span>
            </label>
            <select id="analytics-chain" class="filter-select" title="Vælg kæde til filtrering">
              <option value="">Alle kæder</option>
          ...
          <div class="filter-group">
            <label for="analytics-start" class="filter-label">
              <span class="label-text">Fra dato</span>
              <span class="tooltip-icon" title="Startdato for data-interval">?</span>
            </label>
            <input type="date" id="analytics-start" class="filter-input"
              value="${this._analyticsFilters.startDate || ''}"
              title="Vælg startdato for filtrering">
          </div>

          <div class="filter-group">
            <label for="analytics-end" class="filter-label">
              <span class="label-text">Til dato</span>
              <span class="tooltip-icon" title="Slutdato for data-interval">?</span>
            </label>
            <input type="date" id="analytics-end" class="filter-input"
              value="${this._analyticsFilters.endDate || ''}"
              title="Vælg slutdato for filtrering">
          </div>

          <div class="filter-actions">
            <button class="btn btn-apply" id="analytics-apply" title="Anvend alle valgte filtre på analytik">
              ✓ Anvend filter
            </button>
            <button class="btn btn-secondary" id="analytics-reset" title="Nulstil alle filtre til standard">
              ↺ Nulstil
            </button>
            <button class="btn btn-export" id="analytics-export" title="Eksportér analytik-data til CSV-fil">
              📥 Eksportér
            </button>
          </div>
        </div>
  ...
  _renderMetricsWidget() {
    ...
    const getTrendIcon = (rate) => rate > 80 ? '📈' : rate > 50 ? '→' : '📉';
    const getTrendLabel = (rate) => rate > 80 ? 'Excellent' : rate > 50 ? 'Good' : 'Fair';
      ...
      <div class="metrics-widget">
        <div class="metric-card metric-primary">
          <div class="metric-header">
            ...
            <span class="metric-help" title="Procentdel af succesfuldt gennemførte eksekveringer">ℹ️</span>
          </div>
          ...
          <div class="metric-footer">
            <span class="metric-trend ${successRate > 80 ? 'excellent' : successRate > 50 ? 'good' : 'fair'}">
              ${getTrendIcon(successRate)} ${getTrendLabel(successRate)}
            </span>
          </div>
        </div>
        ...
        <div class="metric-card">
          <div class="metric-header">
            ...
            <span class="metric-help" title="Samlet antal kæde-eksekveringer i perioden">ℹ️</span>
          </div>
          <div class="metric-value">${total}</div>
          <div class="metric-subtext">kæde-kørelser</div>
        </div>
        ...
        <div class="metric-card metric-warning">
          <div class="metric-header">
            ...
            <span class="metric-help" title="Antal mislykkede eksekveringer">ℹ️</span>
          </div>
          <div class="metric-value">${failed}</div>
          <div class="metric-subtext">${failed > 0 ? 'Kræver opmærksomhed' : 'Ingen fejl'}</div>
        </div>
        ...
        <div class="metric-card">
          <div class="metric-header">
            ...
            <span class="metric-help" title="Gennemsnitlig eksekveringstid">ℹ️</span>
          </div>
          <div class="metric-value">${avgDuration}<span class="metric-unit">s</span></div>
          <div class="metric-subtext">per execution</div>
        </div>
      </div>
    `;
  }
  ...
  _renderChainPerformance() {
    ...
    if (!chains.length) {
      return `<div class="empty-state"><p>Ingen kæde-data tilgængelig</p></div>`;
    }
    ...
    return `
      <div class="section">
        <div class="section-header">
          ...
          <p class="section-subtitle" title="Performance-metrics for alle kæder sorteret efter langsomste">
            Sorteret efter langsomste gennemsnitlig varighed
          </p>
        </div>
        <div class="chain-performance-table">
          <div class="table-header">
            ...
            <div class="col-executions" title="Antal eksekveringer">Kørelser</div>
            <div class="col-success" title="Success-rate %">Success</div>
            <div class="col-duration" title="Gennemsnitlig varighed">Varig.</div>
          </div>
          ${chains.map((ch, idx) => {
            const successRate = (ch.success_rate || 0).toFixed(1);
            const statusColor = successRate > 80 ? 'success' : successRate > 50 ? 'warning' : 'error';
            return `
              <div class="table-row" title="Kæde: ${ch.chain_name || ch.chain_id}">
                <div class="col-rank"><span class="rank-badge">${idx + 1}</span></div>
                <div class="col-name">${ch.chain_name || ch.chain_id}</div>
                <div class="col-executions">${ch.executions || 0}</div>
                <div class="col-success">
                  <span class="badge badge-${statusColor}" title="${successRate}% success rate">
                    ${successRate}%
                  </span>
                </div>
